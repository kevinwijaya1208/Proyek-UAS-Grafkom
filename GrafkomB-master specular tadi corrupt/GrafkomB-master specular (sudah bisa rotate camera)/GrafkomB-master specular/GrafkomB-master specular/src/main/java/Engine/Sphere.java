...
public class Sphere extends Circle{
    ...
    public Sphere(List<ShaderModuleData> shaderModuleDataList, List<Vector3f> vertices, Vector4f color, List<Float> centerPoint, Float radiusX, Float radiusY, Float radiusZ, String filename, String texture) {
...
//        System.out.println(maxAABB);
    ...
    public boolean checkCollision(Player player) {
        if (maxAABB.x < player.minAABB.x || minAABB.x > player.maxAABB.x) {
            return false;
        }
        if (maxAABB.y < player.minAABB.y || minAABB.y > player.maxAABB.y) {
            return false;
        }
        if (maxAABB.z < player.minAABB.z || minAABB.z > player.maxAABB.z) {
            return false;
        }
        return true;
    }
